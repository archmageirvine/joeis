 ...
 */
class NevilleThetaS extends CrFunction2Cr {
  ...
  public CR cr(final CR z, final CR m) {
    ...
    final CR a = SQRT_2PI.multiply(qm.pow(Q.ONE_QUARTER)).divide(m.pow(Q.ONE_QUARTER).multiply(CR.ONE.subtract(m).pow(Q.ONE_QUARTER)).multiply(km.sqrt()));
    ...
    return new CR() {
      ...
      protected Z approximate(final int precision) {
        ...
        while (true) {
          ...
          final Z u = scale(t.multiply(s.multiply(2 * k + 1).sin().getApprox(p)), p);
