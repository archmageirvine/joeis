 ...
 */
public class QuadrangulationExtensionFinder {
  ...
  QuadrangulationExtensionFinder(final Plantri plantri, final Numbering numbering, final Marks vertexMarks) {
    mPlantri = plantri;
    ...
    if (nbTot == 1) {
      ...
      for (int i = 0; i < mState.mNumVertices; ++i) {
        if (mDegree[i] >= 4) {
          ...
          if (i == vx) {
            do {
              if (ThreeConnected.willBe3Connected(mDegree, e)) {
                extP1[k++] = e;
              }
              e = e.mNext;
            } while (e != eLast);
            ...
            do {
              if (mVertexMarks.isMarkedVertex(e.mPrev.mEnd) || mVertexMarks.isMarkedVertex(e.mNext.mEnd)
                || mDegree[e.mEnd] >= maxdeg) {
                if (ThreeConnected.willBe3Connected(mDegree, e)) {
                  extP1[k++] = e;
                }
              }
      ...
      for (int i = 0; i < mState.mNumVertices; ++i) {
        if (mDegree[i] >= 4) {
          ...
          Edge e = eLast;
          do {
            if (!ef.isMarkedLo(e) && ThreeConnected.willBe3Connected(mDegree, e)) {
              extP1[k++] = e;
