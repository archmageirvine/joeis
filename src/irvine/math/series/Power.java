...
class Power<E> extends MemoryFunction1<E> implements Series<E> {
  ...
  Power(final Field<E> elementField, final Series<E> s, final long n) {
    if (elementField.zero().equals(s.coeff(0))) {
  ...
  protected E compute(final int m) {
    ...
    final E sum = mElementField.sum(1, m, k -> mElementField.multiply(get(m - k), mElementField.multiply(mS.coeff(k), mElementField.coerce(k * mN - m + k))));
