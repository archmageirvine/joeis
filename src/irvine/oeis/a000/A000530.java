...
/**
 * A000530 Let p(n, s, <code>x)</code> be predicate that number of occurrences of s's in <code>x &gt;= 2*n -</code> the length of the longest sequence of s's in x. Then a(n)=#{x in <code>{0,1}*</code> | x ends in 0 and p(n,0,x) and (there is no prefix y of x such that p(n,0,y) or p(n,1,y))}.
