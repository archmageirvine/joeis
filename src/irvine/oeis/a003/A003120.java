...
public class A003120 implements Sequence {
  ...
  private static Polynomial<Polynomial<Q>> doubleSubstitutePower(final Polynomial<Polynomial<Q>> p, final int power, final int truncate) {
    final ArrayList<Polynomial<Q>> newCoeff = new ArrayList<>();
    for (int k = 0; k <= p.degree(); ++k) {
      newCoeff.add(p.coeff(k).substitutePower(power, truncate));
    }
    return RING_XY.create(newCoeff).substitutePower(power, truncate);
  }

  private static Polynomial<Polynomial<Q>> innerShift(final Polynomial<Polynomial<Q>> p, final int shift) {
    final ArrayList<Polynomial<Q>> newCoeff = new ArrayList<>();
    for (int k = 0; k <= p.degree(); ++k) {
      newCoeff.add(RING_X.shift(p.coeff(k), shift));
    }
  ...
  public Z next() {
    ...
    for (int n = 6; n <= mN; ++n) {
      ...
      for (int k = 1; k <= n + 1; ++k) {
        t6 = RING_XY.add(t6, RING_XY.multiply(doubleSubstitutePower(s, k, n + 1), RING_X.monomial(new Q(1, k), 0)));
      }
      final Polynomial<Polynomial<Q>> t8 = RING_XY.shift(innerShift(expm1(t6, n + 1), 1), -1);
      ...
      s = RING_XY.add(s, innerShift(t13, n));
    }
    final Polynomial<Q> xf1 = RING_XY.eval(RING_XY.shift(s, -1), RING_X.zero());
