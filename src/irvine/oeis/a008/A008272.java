...
public class A008272 extends A008271 {
  ...
  public Z next() {
    ...
    for (int k = 0; k <= mN - 2; ++k) {
      sum = sum.add(Binomial.binomial(mN - 2, k).multiply(Z.TWO.multiply(k).add(3).multiply(k).add(2)).multiply(2L * k + 1).multiply(Functions.FACTORIAL.z(2 * k + 1)).shiftRight(k));
    }
