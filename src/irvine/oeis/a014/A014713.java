...
public class A014713 implements Sequence {
  ...
  public Z next() {
    ...
    final QPolynomial<Z> a = RING.multiply(theta3, theta2z2, mN);
    final QPolynomial<Z> b = RING.multiply(RING.pow(RING.multiply(theta4, theta3z2, mN), 2, mN), Z.FOUR);
    return RING.multiply(RING.pow(a, 6, mN), RING.add(RING.pow(a, 2), b), mN).coeff(mN);
  }
}
