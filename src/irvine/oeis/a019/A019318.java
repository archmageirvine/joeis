...
public class A019318 extends Sequence1 {
  ...
  private Polynomial<Z> p(final long b, final int n) {
    ...
    if ((n & 1) == 0) {
      return RING.add(RING.add(RING.add(RING.pow(c1, (long) n * n, n),
        RING.multiply(RING.multiply(RING.pow(c1, n, n), RING.pow(c2, ((long) n * n - n) / 2, n), n), Z.TWO)),
        RING.multiply(RING.pow(c2, (long) n * n / 2, n), Z.THREE)),
        RING.multiply(RING.pow(c4, (long) n * n / 4, n), Z.TWO));
    } else {
      return RING.add(RING.add(RING.add(RING.pow(c1, (long) n * n, n),
        RING.multiply(RING.multiply(c1, RING.pow(c4, ((long) n * n - 1) / 4, n)), Z.TWO)),
        RING.multiply(c1, RING.pow(c2, ((long) n * n - 1) / 2, n))),
        RING.multiply(RING.multiply(RING.pow(c1, n, n), RING.pow(c2, ((long) n * n - n) / 2, n)), Z.FOUR));
    }
  }
