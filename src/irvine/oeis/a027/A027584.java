...
/**
 * A027584 Sequence satisfies <code>T^2(a)=a</code>, where T is defined below.
 * @author Sean A. Irvine
 */
public class A027584 implements Sequence {
  ...
  static Polynomial<Z> specialPartitionTransform(final Polynomial<Z> a, final int n) {
    ...
    for (final Z z : a) {
      ...
      if (u > prev) { // distinct values
        if ((u & 1) == 0) {
          num = RING.multiply(num, RING.onePlusXToTheN(u), n);  // even u occurs at most one
        } else {
          den = RING.multiply(den, RING.oneMinusXToTheN(u), n); // odd u any number of times
        }
        prev = u;
      }
    }
