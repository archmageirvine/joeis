...
/**
 * A028958 Numbers represented by quadratic form with Gram matrix <code>[ 2, 1; 1, 12 ]</code> (divided by <code>2)</code>.
  ...
  public Z next() {
    while (mX2.compareTo(mForms.first()) <= 0) {
      ...
      for (long y = 0; y <= mX; ++y) {
        ...
        final Z t = y2.multiply(6).add(mX2);
        mForms.add(t.add(s));
