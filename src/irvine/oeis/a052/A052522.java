    ...
    for (int k = 0; k < mN; ++k) {
      for (int j = 0; j <= k; ++j) {
        ...
        for (int l = 0; l <= j; ++l) {
          Z u = Z.ZERO;
          for (int i = 0; i <= l; ++i) {
            u = u.signedAdd(((i + mN + l - 1) & 1) == 0, Functions.FACTORIAL.z(mN + k - 1).multiply(Binomial.binomial(l, i)).multiply(Functions.STIRLING1.z(mN + j - i - l - 1, j - l)).shiftLeft(l - 2L * i).divide(Functions.FACTORIAL.z(mN + j - i - l - 1)));
          }
