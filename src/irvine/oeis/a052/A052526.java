  ...
  public Z next() {
    ...
    for (int k = 0; k < mN; ++k) {
      for (int j = 0; j <= k; ++j) {
        ...
        for (int l = 0; l <= j; ++l) {
          for (int i = 0; i <= l; ++i) {
            t = t.signedAdd((l & 1) == 0, mF.factorial(mN + k - 1).multiply(Stirling.secondKind(mN + j - i - l - 1, j - l)).shiftLeft(l - 2L * i).divide(mF.factorial(mN + j - i - l - 1)).divide(mF.factorial(i)).divide(mF.factorial(l - i)));
          }
        }
