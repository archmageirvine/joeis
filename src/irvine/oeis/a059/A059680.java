 ...
 */
public class A059680 extends Sequence3 {
  ...
  private static final String TRANSITIONS =
      ...
      "####.#.##.##.##.#.##";  // 20
  // @formatter:on
  // Number of set squares in each of the 20 animals
    ...
    for (int k = 0, i = 0; k < ANIMALS; ++k) {
      ...
      for (int j = 0; j < ANIMALS; ++j, ++i) {
        if (TRANSITIONS.charAt(i) == '#') {
          PSI.set(k, j, RING.monomial(Z.ONE, CONTENT[j]));
          //PSI.set(j, k, RING.monomial(Z.ONE, WEIGHTS[j])); // equivalent to transpose of Psi
        }
      }
    }
  ...
  // todo could be some ordering transpose issue around S, P, PSI
  // todo could be errors in PSI
  // todo perhaps do this on smaller case first to verify procedure
  ...
  private final MemoryFunction1<Polynomial<Z>> mDStar = new MemoryFunction1<>() {
    ...
    protected Polynomial<Z> compute(final int n) {
      ...
      //final Matrix<Polynomial<Z>> p = MAT_RING.multiply(MAT_RING.multiply(P, MAT_RING.pow(PSI.transpose(), mM - 1)), S);
      System.out.println("Poly: " + p.get(0, 0));
      return p.get(0, 0);
    }
  };
