 ...
 */
public class A060879 extends Sequence1 {
  ...
  @Override
  public Z next() {
    if (mA.isEmpty() || mBase * mBase * mBase * mBase * mBase <= mA.first()) {
      for (long a = 1; a < mBase; ++a) {
        for (long b = 0; b < mBase; ++b) {
          for (long c = 0; c < mBase; ++c) {
            for (long d = 0; d < mBase; ++d) {
              for (long e = 0; e < mBase; ++e) {
                mA.add(((((((((a * mBase + b) * mBase + c) * mBase + d) * mBase + e) * mBase + d) * mBase) + c) * mBase + b) * mBase + a);
              }
            }
          }
        }
