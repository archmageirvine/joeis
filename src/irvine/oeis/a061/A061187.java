 ...
 */
public class A061187 extends Sequence0 {
  ...
  public Z next() {
    if (++mM > mN + 1 + mN / 2) {
      ++mN;
      mM = 0;
      final int lim = 2 * (mN + 2 + mN / 2);
      final Polynomial<Q> a = RING.pow(C1, mN + 1, lim);
      final Polynomial<Q> b = RING.pow(C2, mN + 1, lim);
      mRow = RING.multiply(RING.subtract(a, b), Q.HALF);
    }
    return mRow.coeff(2 * mM + 1).toZ();
  }
}
