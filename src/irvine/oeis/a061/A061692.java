...
public class A061692 extends Sequence1 {
  ...
  private final MemoryFunction1<Polynomial<Q>> mB = new MemoryFunction1<>() {
    ...
    protected Polynomial<Q> compute(final int n) {
      return n == 0
        ? RING.one()
        : RING.sum(1, n, i -> RING.multiply(get(n - i).shift(1), new Q(Z.ONE, MemoryFactorial.SINGLETON.factorial(i).pow(3))));
    }
  };

  protected Z t(final int n, final int m) {
    return mB.get(n).coeff(m)
      .multiply(MemoryFactorial.SINGLETON.factorial(n).pow(3))
