 ...
 */
public class A062190 extends Sequence0 {
  ...
  public Z next() {
    if (++mM > mRow.degree()) {
      ...
      mRow = RING.zero();
      for (int k = 0; k <= mN; ++k) {
        final Z c = Binomial.binomial(mN, k).multiply(F.factorial(2 * mN + 5 - k)).divide(F.factorial(mN - k));
        mRow = RING.add(mRow, RING.multiply(RING.pow(C, k), c).shift(mN - k));
      }
      mRow = RING.divide(mRow, F.factorial(mN + 5));
      mM = 0;
    }
