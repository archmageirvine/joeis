...
/**
 * A077370 Largest n-digit triangular number whose external digits as well as internal digits form a triangular number, or 0 if no such number exists.
 * @author Sean A. Irvine
 */
public class A077370 extends Sequence1 {
  ...
  public Z next() {
    if (++mN <= 1) {
      return mN == 0 ? Z.SIX : Z.valueOf(91);
    }
    final Z min = Z.TEN.pow(mN);
    final Z max = min.multiply(10);
    Z ti = Functions.TRINV.z(max);
    while (true) {
