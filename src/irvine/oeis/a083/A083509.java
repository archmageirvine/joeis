...
public class A083509 extends Sequence1 {
  ...
  // This is the same implementation as A036057, but will some ugly changes
  // to ensure representations involving "^" are found.
  ...
  private static class Node {
    ...
    public boolean equals(final Object a) {
      return a instanceof Node
        && mUsed == ((Node) a).mUsed
        && Math.abs(mValue - ((Node) a).mValue) < TOLERANCE
        // Extra evilness needed for this sequence
        && mDesc.contains("^") == ((Node) a).mDesc.contains("^");
    }
    @Override
    public int hashCode() {
  ...
  private void insert(final double v, final int used, final String desc) {
    ...
    if (used == mMaxUsed) {
      if (integer && vv == mN && isNontrivial(n)) {
        if (mAnswer == null || !mAnswer.mDesc.contains("^")) {
          mAnswer = n;
        }
      }
      return;
  ...
  private boolean isFriedman(final int n) {
    ...
    crunch(digits);
    return mAnswer != null && mAnswer.mDesc.contains("^");
  }
