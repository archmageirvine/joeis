package irvine.oeis.a084;
// Generated by gen_seq4.pl manch4/tablamb at 2024-03-12 13:36

import irvine.math.factorial.MemoryFactorial;
import irvine.math.z.Binomial;
import irvine.math.z.Z;
import irvine.oeis.triangle.LambdaTable;

/**
 * A084950 Array of coefficients of denominator polynomials of the n-th approximation of the continued fraction x/(1+x/(2+x/(3+..., related to Laguerre polynomial coefficients.
 * @author Georg Fischer
 */
public class A084950 extends LambdaTable {

  /** Construct the sequence. */
  public A084950() {
    super(0, 0, n -> new Integer[]{0, n / 2}, (n, k) -> n <= 1 ? Z.ONE : MemoryFactorial.SINGLETON.factorial(n - k).multiply(Binomial.binomial(n - k, k)).divide(MemoryFactorial.SINGLETON.factorial(k)));
  }
}
