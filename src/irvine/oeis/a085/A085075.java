...
/**
 * A085075 Smallest number a(n) == -1 (mod n) such that the prime signature of n and a(n) is the same, or 0 if no such number exists.
 * @author Sean A. Irvine
 */
public class A085075 extends Sequence2 implements Conjectural {
  ...
  public Z next() {
    ...
    if (Predicates.PRIME_POWER.is(mN)) {
      ...
      while (true) {
        ...
        if (LongUtils.modPow(k, e, mN) == mN - 1) {
          return Z.valueOf(k).pow(e);
        }
        if (k > HEURISTIC_LIMIT) {
          return Z.ZERO;
        }
      }
    }
