...
/**
 * A086610 Triangle of coefficients, read by rows, where T(n,k) is the coefficient of x^n*y^k in f(x,y) that satisfies f(x,y) = 1/(1-x) - x^2/(1-x)^2 + xy*f(x,y)^2.
 * @author Sean A. Irvine
 */
public class A086610 extends Sequence0 {
  ...
  public Z next() {
    if (++mM > mN) {
      ...
      mF = ring.add(ring.series(ring.one(), ring.oneMinusXToTheN(1), mN),
        ring.series(ring.monomial(Polynomial.create(-1), 2), ring.pow(ring.oneMinusXToTheN(1), 2, mN), mN),
