...
/**
 * A094501 Smallest number that requires n iterations of the sum of digits of the divisors <code>(A034690)</code> to reach 15.
...
public class A094501 implements Sequence {
  ...
  private long divisorDigitSum(final long m) {
    ...
    long sum = 0;
    for (final Z t : Cheetah.factor(m).divisors()) {
      sum += ZUtils.digitSum(t);
    }
    return sum;
  }
