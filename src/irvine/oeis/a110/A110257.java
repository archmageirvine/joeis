package irvine.oeis.a110;
// Generated by gen_seq4.pl 2024-06-24/lambdan at 2024-06-24 23:40

import irvine.math.function.Functions;
import irvine.math.z.Binomial;
import irvine.math.z.Z;
import irvine.oeis.LambdaSequence;

/**
 * A110257 Numerators in the coefficients that form the odd-indexed partial quotients of the continued fraction representation of the inverse tangent of 1/x.
 * @author Georg Fischer
 */
public class A110257 extends LambdaSequence {

  /** Construct the sequence. */
  public A110257() {
    super(1, n -> Z.valueOf(4L * n - 3).multiply(Binomial.binomial(2L * n - 2, n - 1).square()).divide(Z.FOUR.pow(Functions.DIGIT_SUM.i(2, n - 1))));
  }
}
