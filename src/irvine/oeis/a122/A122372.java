...
/**
 * A122372 Dimension of 8-variable non-commutative harmonics (twisted derivative). The dimension of the space of non-commutative polynomials in 8 variables which are killed by all symmetric differential operators (where for a monomial w, d_{xi} <code>( xi</code> w <code>) =</code> w and d_{xi} <code>( xj</code> w <code>) = 0</code> for <code>i/=j)</code>.
