...
/**
 * A134168 Let P(A) be the power set of an n-element set A. Then <code>a(n) =</code> the number of pairs of elements <code>{x,y}</code> of P(A) for which either <code>0) x</code> and y are disjoint and for which either x is a subset of y or y is a subset of x, or <code>1) x</code> and y are intersecting but for which x is not a subset of y and y is not a subset of x, or <code>2) x</code> and y are intersecting and for which either x is a proper subset of y or y is a proper subset of x, or <code>3) x = y</code>.
