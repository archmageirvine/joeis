package irvine.oeis.a151;
// Generated by gen_seq4.pl build/simple at 2022-06-14 21:39

import irvine.math.z.Binomial;
import irvine.math.z.Z;
import irvine.oeis.Sequence1;

/**
 * A151657 Number of permutations of 7 indistinguishable copies of 1..n with exactly 3 adjacent element pairs in decreasing order.
 * @author Georg Fischer
 */
public class A151657 extends Sequence1 {

  private int mN = 0;

  @Override
  public Z next() {
    ++mN;
    return Z.valueOf(120).pow(mN).subtract(Z.valueOf(36).pow(mN).multiply(7L * mN + 1)).add(Binomial.binomial(7L * mN + 1, 2).multiply(Z.EIGHT.pow(mN))).subtract(Binomial.binomial(7L * mN + 1, 3));
  }
}
