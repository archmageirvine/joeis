...
/**
 * A151739 Primes which are the sum of ten consecutive composite numbers.
 * @author Sean A. Irvine
 */
public class A151739 extends Sequence1 {
  ...
  public Z next() {
    while (true) {
      ...
      mK = mL;
      mL = mV.next();
      final Z sum = c.add(mD).add(mE).add(mF).add(mG).add(mH).add(mI).add(mJ).add(mK).add(mL);
