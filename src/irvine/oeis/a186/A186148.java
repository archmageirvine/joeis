...
public class A186148 extends AbstractSequence {
   ...
   */
  public A186148(final int offset, final Q d, final Q u, final Q v, final int p, final int q) {
    super(offset);
    mN = offset - 1;
    mD = d;
    mU = CR.valueOf(u);
  ...
  */
  ...
  public Z next() {
    ++mN;
    return mU.multiply(Z.valueOf(mN).pow(mP)).subtract(mD).divide(mV).pow(new Q(1, mQ)).floor().add(mN);
  }
}
