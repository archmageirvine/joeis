package irvine.oeis.a283;
// Generated by gen_seq4.pl 2024-03-07/lambdan at 2024-03-07 22:01

import irvine.math.function.Functions;
import irvine.math.q.Q;
import irvine.math.q.Rationals;
import irvine.math.z.Binomial;
import irvine.math.z.Z;
import irvine.oeis.LambdaSequence;

/**
 * A283153 Number of set partitions of unique elements from an n X 4 matrix where elements from the same row may not be in the same partition.
 * @author Georg Fischer
 */
public class A283153 extends LambdaSequence {

  /** Construct the sequence. */
  public A283153() {
    super(1, n -> {
      return Rationals.SINGLETON.sum(1, 4 * n, p -> {
        return new Q(Binomial.binomial(p, 4).pow(n), Functions.FACTORIAL.z(p)).multiply(Rationals.SINGLETON.sum(0, 4 * n - p, k -> {
          return new Q(Z.NEG_ONE.pow(k), Functions.FACTORIAL.z(k));
        }));
      }).multiply(Functions.FACTORIAL.z(4).pow(n)).num();
    });
  }
}
