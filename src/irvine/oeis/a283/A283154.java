...
public class A283154 extends LambdaSequence {
  ...
  public A283154() {
    super(1, n -> {
      return Rationals.SINGLETON.sum(1, 5 * n, p -> {
        return new Q(Binomial.binomial(p, 5).pow(n), Functions.FACTORIAL.z(p)).multiply(Rationals.SINGLETON.sum(0, 5 * n - p, k -> {
          return new Q(Z.NEG_ONE.pow(k), Functions.FACTORIAL.z(k));
        }));
      }).multiply(Functions.FACTORIAL.z(5).pow(n)).num();
    });
  }
}
