...
public class A289064 extends Sequence0 {
  ...
  protected int mN;
  private final ArrayList<Zi> mLst = new ArrayList<>();
  ...
  public A289064(final int a0, final int a1Re, final int a1Im, final int ri) {
    mA0 = new Zi(a0);
    mA1 = new Zi(a1Re, a1Im);
    mRi = ri;
    mN = -1;
  }

  protected Zi computeNext() {
    ++mN;
    if (mN == 0) {
      return mA0;
    } else if (mN == 1) {
    ...
    for (int k = mN - 2; k >= 0; --k) {
      sum = GI.add(sum, GI.multiply(a(k), GI.multiply(a(mN - 1 - k), new Zi(Binomial.binomial(mN - 2, k)))));
    }
    return sum;
  }
