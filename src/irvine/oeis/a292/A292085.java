 ...
 */
public class A292085 extends UpperLeftTriangle implements DirectArray {
  ...
  */
  final MemoryFunctionInt4<Z> mB = new MemoryFunctionInt4<>() {
    ...
    protected Z compute(final int n, final int i, final int v, final int k) {
      return
        n == 0 ? (v == 0 ? Z.ONE : Z.ZERO) : (i < 1 || v < 1 || n < v ? Z.ZERO : (v == n ? Z.ONE : Integers.SINGLETON.sum(0, Functions.MIN.i(n / i, v), j -> Binomial.binomial(mA.get(i, k).add(j - 1), j).multiply(get(n - i * j, i - 1, v - j, k)))));
    }
  };
