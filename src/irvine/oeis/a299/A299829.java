package irvine.oeis.a299;
// Generated by gen_seq4.pl rootet at 2023-01-22 20:14

import irvine.oeis.SkipSequence;
import irvine.oeis.a000.A000521;
import irvine.oeis.transform.RootSequence;

/**
 * A299829 Coefficients in expansion of (q*j(q))^(-5/24) where j(q) is the elliptic modular invariant (A000521).
 * Coefficients in expansion of (q*j(q))^(-5/24) where j(q) is the elliptic modular invariant (A000521).
 * @author Georg Fischer
 */
public class A299829 extends RootSequence {

  /** Construct the sequence. */
  public A299829() {
    super(0, new SkipSequence(new A000521(), 1), -5, 24);
  }
}
