package irvine.oeis.a327;
// Generated by gen_seq4.pl egfu at 2021-12-06 15:00

import irvine.math.polynomial.Polynomial;
import irvine.math.q.Q;
import irvine.oeis.ExponentialGeneratingFunction;

/**
 * A327505 Number of set partitions of [n] where each subset is again partitioned into four nonempty subsets.
 * E.g.f.: exp((exp(x)-1)^4/24)
 * @author Georg Fischer
 */
public class A327505 extends ExponentialGeneratingFunction {

  /** Construct the sequence. */
  public A327505() {
    super(0);
  }

  @Override
  public Polynomial<Q> compute(final int mN) {
    return RING.exp(RING.series(RING.pow(RING.subtract(RING.exp(RING.x(), mN), RING.one()), 4, mN), RING.monomial(new Q(24), 0), mN), mN);
  }
}
