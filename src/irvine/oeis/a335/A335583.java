package irvine.oeis.a335;
// Generated by gen_seq4.pl wparts/wpartsum at 2025-03-11 22:45

import irvine.math.z.Integers;
import irvine.math.z.Z;
import irvine.oeis.LambdaSequence;

/**
 * A335583 Number of 1&apos;s in the partitions of n into 4 parts.
 * @author Georg Fischer
 */
public class A335583 extends LambdaSequence {

  /** Construct the sequence. */
  public A335583() {
    super(0, n -> Integers.SINGLETON.sum(1, n / 4, k -> Integers.SINGLETON.sum(k, (n - k) / 3, j -> Integers.SINGLETON.sum(j, (n - j - k) / 2, i -> Z.valueOf((k == 1 ? 1 : 0) + (j == 1 ? 1 : 0) + (i == 1 ? 1 : 0) + (n - i - j - k == 1 ? 1 : 0))))));
  }
}
