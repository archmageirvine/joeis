...
// BgfToArray(gf, N, M)={matrix(N+1, M+1, n, m, polcoeff(polcoeff(gf, n-1, x), m-1, y))}
  ...
  Polynomial<Polynomial<Z>> orientedStrongTriangsGf(final int m, final int n) {
    ...
    final Polynomial<Polynomial<Z>> e = PolynomialUtils.innerShift(ring, ring.sum(3, m, k -> ring.multiply(bgfRaise(bgfTrim(gr, (int) (m / k + 1), (int) (n / k + 1)), k.intValue()), Polynomial.create(Functions.PHI.l(k)))), 1);
