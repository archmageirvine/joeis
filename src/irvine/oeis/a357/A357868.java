package irvine.oeis.a357;
// Generated by gen_seq4.pl 2025-02-10.ack/arrlamb at 2025-02-10 21:26

import irvine.math.function.Functions;
import irvine.math.z.Integers;
import irvine.oeis.triangle.LambdaArray;

/**
 * A357868 Square array T(n,k), n &gt;= 0, k &gt;= 0, read by antidiagonals downwards, where T(n,k) = Sum_{j=0..n} (k*j)!* Stirling2(n,k*j).
 * @author Georg Fischer
 */
public class A357868 extends LambdaArray {

  /** Construct the sequence. */
  public A357868() {
    super(0, 0, 0, -1, (n, k) -> Integers.SINGLETON.sum(0, n, j -> Functions.FACTORIAL.z(k * j).multiply(Functions.STIRLING2.z(n, (long) k * j))));
  }
}
