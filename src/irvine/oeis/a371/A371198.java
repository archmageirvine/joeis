  ...
  public A371198() {
    super(0, n -> Rationals.SINGLETON.sum(0, n, j -> {
      return Rationals.SINGLETON.sum(0, j / 3, k -> {
        return new Q(Binomial.binomial(j - 2 * k, n - j - k).multiply(Functions.STIRLING1.z(j - 2 * k, (long) k).abs()).multiply(Functions.FACTORIAL.z(k)), Functions.FACTORIAL.z(j - 2 * k));
