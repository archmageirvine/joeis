...
/**
 * A395692 Triangle read by rows: T(n,k) = number of labeled 3-multigraphs on n vertices with clique number exactly k.
 * @author Sean A. Irvine
 */
public class A395692 extends Sequence1 {
  ...
  public Z next() {
    if (++mM > mN) {
      ...
      new ParallelGenerateGraphsSequence(mN, mN - 1, 0, 0, () -> graph -> {
        final Z labellings = Functions.FACTORIAL.z(graph.order()).divide(GraphUtils.automorphismSize(graph)).multiply(Z.THREE.pow(maxe - graph.size()));
