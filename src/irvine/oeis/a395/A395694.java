 ...
 */
public class A395694 extends Sequence1 {
  ...
  public Z next() {
    if (++mM > mN) {
      ...
      new ParallelGenerateGraphsSequence(mN, mN - 1, 0, 0, () -> graph -> {
        final Z labellings = Functions.FACTORIAL.z(graph.order()).divide(GraphUtils.automorphismSize(graph)).multiply(Z.FIVE.pow(maxe - graph.size()));
