...
public class TriangleSelector extends AbstractSequence {
  ...
  private final boolean mHasRAM; // whether the underlying sequence has a random access method
  private Triangle mTriangle; // <code>mSeq</code> casted into a Triangle when <code>mHasRAM = true</code>
   ...
   */
  public TriangleSelector(final Sequence seq, final int row0, final Function<Integer, int[]> select) {
   ...
   * @param select function giving a pair of indexes (coordinates) into the underlying triangle
   */
  public TriangleSelector(final int offset, final Sequence seq, final int row0, final Function<Integer, int[]> select) {
   ...
   * @param select function giving a pair of indexes (coordinates) into the underlying triangle
   */
  public TriangleSelector(final Sequence seq, final int row0, final int col0, final Function<Integer, int[]> select) {
  ...
  public TriangleSelector(final int offset, final Sequence seq, final int row0, final int col0, final Function<Integer, int[]> select, final boolean hasRAM) {
    ...
    mSelect = select;
    mHasRAM = hasRAM;
    if (mHasRAM && mSeq instanceof Triangle) {
      mTriangle = (Triangle) mSeq;
    }
    mN = offset - 1;
