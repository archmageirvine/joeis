 ...
 */
public class LongDynamicIntArrayTest extends TestCase {

  public void testIntPrimitive() {
    final LongDynamicIntArray la = new LongDynamicIntArray();
    ...
    try {
      ...
      assertEquals("Cannot truncate to negative length.", e.getMessage());
    }
    la.set(5, 5);
    ...
    int max = -1;
    int mv = 0;
    for (int i = 0; i < 500000; i += 101) {
      final int j = r.nextInt(1000000);
      final int v = 1 + r.nextInt(1000000);
      la.set(j, v);
  ...
  public void testBoundaryTruncate() {
    final long ll = 1L << 20;
    final LongDynamicIntArray a = new LongDynamicIntArray();
    a.set(ll - 1, 42);
    a.set(ll, 43);
    a.set(ll + 1, 44);
    a.truncate(ll);
  ...
  public void testNastyArrayLength() {
    final LongDynamicIntArray m = new LongDynamicIntArray();
    m.set((1 << 21) - 1, 1);
    ...
    assertEquals(1 << 20, ((int[]) x.get(0)).length);
    assertEquals(1 << 20, ((int[]) x.get(1)).length);
    m.set((1 << 21) + 1, 2);
